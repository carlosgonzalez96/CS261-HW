...
void _adjustHeap(DynArr *heap, int max, int pos)
{
   /* TODO */
	 ...
	 if (rghtIdx < max) {
	 	smallerIdx = _smallerIndexHeap(heap, leftIdx, rghtIdx);
	 	if (compare(getDynArr(heap, smallerIdx), getDynArr(heap, pos)) == -1) {
	 		swapDynArr(heap, pos, smallerIdx);
...
{
    /* TODO */
		int highpos = heap->size - 1;
		int i;
		for(i = (highpos - 1) / 2; i >= 0; i--) {
			_adjustHeap(heap, highpos, i);
		}
}
/*
...
void sortHeap(DynArr *heap)
{
   /* TODO */
	 ...
	 for(highpos = heap->size - 1; highpos >= 0; highpos--){
		 	swapDynArr(heap, highpos, 0);
			_adjustHeap(heap, highpos - 1, 0);
		}
